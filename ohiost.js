...
(async () => {
  ...
  for(var x = 0;x < 2;x++){
    await page.goto("https://www.osu.edu/findpeople/");
    await page.click("input#filterOnlyStudents");
    const curr = names[x]
    console.log("x is ",x)
    ...
    const data = await page.evaluate(() => {
      const tds = Array.from(document.querySelectorAll('td.record-data-email'))
    ...
    fs.writeFile("ohiostate.txt", emails, function(error){
