...
(async () => {
  ...
  for(var x = 0;x < names.length;x++){
    try{
      const curr = names[x]
      await page.goto("https://search.k-state.edu/?qt="+curr+"&curtab=1");
      console.log("x is ",x)
      console.log("curr is ",curr)
      // Type in name and click
      await page.waitForSelector('div.peopleResult', {timeout: 8000});
      ...
      for (var i = 0; i<data.length; i++){
        if(data[i].includes("@k-state.edu")){
          emails.push(data[i]);
        }
      }
    } catch(E) {continue;}
    ...
    fs.writeFile("KState_2020_December.txt", emails, function(error){
