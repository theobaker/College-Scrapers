...
(async () => {
  ...
  for(var x = 0;x < names.length;x++){
    ...
    console.log("x is ",x)
    try{
      await page.goto("https://www.utdallas.edu/directory/");
      await page.waitForSelector('input#dirSearch');
    } catch(E) {
      continue;
    }
    // Type in name and click
    await page.select("select#dirAffil", "student")
    ...
    try{
      ...
      await page.waitForSelector('div.resultPage');
    } catch(E) {
    ...
    for (var i = 0; i<data.length; i++){
      if(data[i].includes("@UTDallas.edu")){
        emails.push(data[i]);
      }
    }
    ...
    fs.writeFile("UTDallas_2021_January.txt", emails, function(error){
