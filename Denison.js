...
(async () => {
  ...
  for(var x = 0;x < names.length;x++){
    await page.goto("https://denison.edu/a-to-z/students");
    await page.waitFor(400)
    const curr = names[x]
    console.log("x is ",x)
    console.log("curr is ",curr)
    // Type in name and click
    await page.click("input#edit-field-last-name-value",{clickcount: 3})
  ...
  for (var i = 0; i<data.length; i++){
    ...
    fs.writeFile("Denison_2020_November.txt", emails, function(error){
